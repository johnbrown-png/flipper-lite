...
class MathVisualGenerator:
    ...
    def __init__(self, default_width=800, default_height=400):
        ...
        try:
            ...
            # Larger fonts for part-whole models: 61→79, 42→55 (+30%)
            self.font_large_pw = ImageFont.truetype("arial.ttf", 79)
            self.font_medium_pw = ImageFont.truetype("arial.ttf", 55)
        except:
    ...
    def generate(self, visual_type: str, params: Dict[str, Any]) -> Image.Image:
        ...
        if visual_type == 'base10_blocks':
            ...
            # Bar models are proportional visuals. For 4-digit numbers (>= 1000),
            # small place values like units and tens render as invisible slivers
            # that fail to communicate place-value structure. Suppress imagery
            # for totals of 1000 or greater.
            total = params.get('total', 0)
            if total >= 1000:
                return None
